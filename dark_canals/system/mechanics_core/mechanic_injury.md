...
## Injury Effects
When a character recives an Injuries of Severe level or above (3+), they must pass an [[skill_endure|ENDURE]] [[mechanic_rollResolution|Roll]] or also recive the corresponding Injury Effect.

### Serious Injury: Attribute Damage
Hit Locations have an associated [[mechanic_attributes|Attribute]] ([[attribute_strength|STRENGTH]] or [[attribute_agility|AGILITY]]).

Each time a new Injury above Minor Level is marked, the associated [[mechanic_attributes|Attribute]] recives [[mechanic_attributeDamage|Attribute Damage]].

### Severe Injury: Effect vs. Hit Location

| Location | Effect         | Description                                  |
| -------- | -------------- | -------------------------------------------- |
| `HEAD`   | Knock out      | Become [[mechanic_fallUnconscious\|Unconscious]].         |
...
- If a leg, it limits movement to [[combat_distance|Half a Chain (within that zone)]].
...
- A different PC must succeed on a [[skill_surgery|SURGERY]] [[mechanic_rollResolution|Roll]].
- [[item_bandages|Bandages]] must be used as part of the action.
- Any number of [[skill_surgery|SURGERY]] rolls can be attempted, but:
	- A single [[skill_surgery|SURGERY]] [[mechanic_rollResolution|Roll]] can be made per [[combat_turn|Combat Turn]].
	- Between every [[skill_surgery|SURGERY]] [[mechanic_rollResolution|Roll]] and the next one, the wounded PC must have passed one [[skill_endure|ENDURE]] [[mechanic_rollResolution|Roll]] in their [[combat_turn|Combat Turn]].
...
### Head or Torso
Automatic [[mechanic_death|Death]].

### Limb
Destroyed limb. Counts as a Lethal Injury.

#### Attribute Effect
Reduce the maximum for the relevant [[mechanic_attributes|Attribute]]. If it gets reduced to less than two points, the PC is crippled, and cannot make [[mechanic_rollResolution|Rolls]] of the associated [[mechanic_skills|Skills]] unless they have at least one point in them. It is still possible to [[mechanic_skillTraining|Train]] to be able to use them.

Any previous Injuries on that limb still need to heal to recover the pertinent [[mechanic_attributeDamage|Attribute Damage]].
